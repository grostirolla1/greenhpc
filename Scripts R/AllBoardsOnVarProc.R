...
ggplot(finalDf, aes(TempoNorm,ConsumoNorm)) +
    ...
    ylim(0, 1) +
    pedro_theme +
    ggsave("2_ondes_tempoxconsumo-relativo.pdf")
...
ggplot(dfSumm, aes(Procs)) +
   ...
   # geom_ribbon(aes(ymin=MTempo - ICTempo, ymax=MTempo + ICTempo), alpha=0.4) +
    ...
    scale_x_continuous(breaks = round(seq(min(dfSumm$Procs), max(dfSumm$Procs), by = 1),1)) +
    pedro_theme + 
    ggsave("2_ondes_tempoxprocs.pdf", width=10,height=6.2)
...
ggplot(dfSumm, aes(Procs)) +
    ...
    xlab("\nnodes") +
    xlim(1, 10) +
    ylim(0, max(dfSumm$MConsumo)+400) +
    pedro_theme +
    scale_x_continuous(breaks = round(seq(min(dfSumm$Procs), max(dfSumm$Procs), by = 1),1)) +
    ggsave("2_ondes_consumoxprocs.pdf", width=10,height=6)
...
#PLOT kJ
    ...
    xlab("\nnodes") +
    xlim(1, 10) +
    ylim(0, max(dfSumm$MConsumo/1000)+1) +
    pedro_theme +
    scale_x_continuous(breaks = round(seq(min(dfSumm$Procs), max(dfSumm$Procs), by = 1),1)) +
    ggsave("2_ondes_consumoxprocs.pdf", width=10,height=6.2)
